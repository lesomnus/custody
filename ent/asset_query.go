...
func (_q *AssetQuery) QueryKeeper() *HolderQuery {
...
func (_q *AssetQuery) Clone() *AssetQuery {
	...
	return &AssetQuery{
		...
		withKeeper: _q.withKeeper.Clone(),
...
// WithKeeper tells the query-builder to eager-load the nodes that are connected to
// the "keeper" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AssetQuery) WithKeeper(opts ...func(*HolderQuery)) *AssetQuery {
	query := (&HolderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AssetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Asset, error) {
	...
	if query := _q.withKeeper; query != nil {
		if err := _q.loadKeeper(ctx, query, nodes, nil,
			func(n *Asset, e *Holder) { n.Edges.Keeper = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AssetQuery) loadKeeper(ctx context.Context, query *HolderQuery, nodes []*Asset, init func(*Asset), assign func(*Asset, *Holder)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Asset)
	for i := range nodes {
