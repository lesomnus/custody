...
// Policy is what this deployment answers "who may see whose rows" with, and it
// is the **only** difference between the two binaries this app builds.
//
// `cmd/custody` leaves it nil, and nil is not a weaker policy -- it is the
// absence of one. `gate.Decide` then answers with the caller's own tenant, and
// there is no code path on that server that answers anything wider. A forged
// headquarters credential arriving at the public address has nothing to call.
//
// `cmd/custody-admin` passes `custody.Hq{...}`, and is deployed where only the
...
//
// The policy is resolved here rather than when the command tree is built,
// because it comes out of the configuration and the configuration is read when
// a command runs.
func NewCmdServe(c *Config, policy Policy) *xli.Command {
	return &xli.Command{
		Name:  "serve",
