...
// payday: v0.0.0-20260811164558-11e55c698d9e
...
const Payday = "v0.0.0-20260811164558-11e55c698d9e"
...
type gateAsset struct {
	Gate
	api.AssetServiceServer
}

func (s Gate) Asset() api.AssetServiceServer {
	return gateAsset{s, s.Next().Asset()}
}

// Add refuses a Asset put into a Tenant this caller cannot see.
//
...
// should not be told.
func (s gateAsset) Add(ctx context.Context, req *api.AssetAddRequest) (*api.Asset, error) {
	...
	return s.AssetServiceServer.Add(ctx, req)
}
...
func subject(ctx context.Context, s bare.Server, key pdid.Id) (uuid.UUID, []byte, error) {
	switch key.Domain() {
	case AssetDomain:
		row, err := s.Asset().Get(ctx, api.AssetGetRequest_builder{
			Ref: api.AssetRef_builder{Id: key.Bytes()}.Build(),
		}.Build())
		if err != nil {
			if status.Code(err) == codes.NotFound {
