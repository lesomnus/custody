...
func (Unscoped) AssetScope(_ context.Context) (predicate.Asset, error) {
	return nil, nil
}
...
func (ss Scopes) AssetScope(ctx context.Context) (predicate.Asset, error) {
	ps := make([]predicate.Asset, 0, len(ss))
	for _, s := range ss {
		p, err := s.AssetScope(ctx)
		if err != nil {
			return nil, err
		}
		if p == nil {
